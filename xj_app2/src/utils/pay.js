...
const pay = {
  // 支付
  pay: function(style, t) {
    // let apptype = $utils.app.type
    t.paytype = "app";
    console.log(style, t, 9999);
    switch (style) {
      case "weixin": // 微信
      case "wechat": // 微信
        return this.wechat(t);
      case "alipay": // 支付宝
        return this.alipay(t);
    }
  },
  // 支付完成
  complete: function(type) {
    ...
    Taro.redirectTo({
      url: `/pages/order/result/index?type=${type}`
    });
    // $utils.auth.sync();
  },
  // 支付宝支付
  alipay: function(t) {
    ...
    v.app_type = "app";
    v.paytype = "app";
    this.getPayinfo("payAlipayPay", v).then(paydata => {
      console.log("pay paydata", paydata);
      // wap
      if (v.paytype === "wap") {
        // document.write(paydata)
        Linking.openURL(v.pa);
      } else if (v.paytype === "app") {
        // 服务器端生成支付数据payinfo
        const payInfo = (paydata && paydata.data) || "";
        try {
          console.log("alipay payInfo ", payInfo);
          // 支付宝插件
          AlipaySdk.pay(payInfo)
            .then(e => {
  ...
  wechat: function(t) {
    ...
    this.getPayinfo("payAlipayPay", v).then(paydata => {
      console.log("pay paydata", paydata, v);
      // wap
      if (v.app_type === "wap") {
        ...
        Taro.requestPayment({
          ...
          fail: function(err) {
            console.log(err);
            global.$utils.toast.text("支付已取消");
            that.complete(2);
          }
        });
      }
      Taro.eventCenter.trigger("refreshOrderList", true);
    });
  },
  // 获取支付数据
  getPayinfo: (urltype, t) => {
    const v = {};
    v.pay_type = t.pay_type;
    v.out_trade_no = t.out_trade_no;
    v.app_type = t.app_type;
    // 异步处理
