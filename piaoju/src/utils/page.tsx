 ...
 */
export function showDrawer(dom: ReactNode, params: DrawerProps): React.RefObject<CommonRefObj> {
  ...
  ReactDOM.render(
    <RenderDrawer ref={ref} node={node} {...params}>
      {dom}
    </RenderDrawer>,
    node,
  );
  // const xx = ReactDOM.createPortal(<RenderDrawer {...params}>{dom}</RenderDrawer>, document.body);
 ...
 * @return 返回引用ref，可以调用close方法关闭
 */
export function showModal(dom: ReactNode, params: ModalProps): React.RefObject<CommonRefObj> {
  const ref = React.createRef<any>();
  const id = new Date().getTime();
  const node = document.createElement('div');
  node.setAttribute('id', '' + id);
  ...
  ReactDOM.render(
    <RenderModal ref={ref} node={node} {...params}>
      {dom}
    </RenderModal>,
    node,
  );
  // const xx = ReactDOM.createPortal(<RenderModal {...params}>{dom}</RenderModal>, document.body);
  // ReactDOM.render(<>{xx.children}</>, node);
  return ref;
}
