...
import { pjForeignData, pjForeignupdate, pjForeignadd } from '@/services/ant-design-pro/pjForeign';
...
const Page: React.FC<any> = (props) => {
  ...
  const initForms = async () => {
    if (props.action === 'edit') {
      const result: any = await pjForeignData(props.id);
  ...
  const onSubmits = async (value: any) => {
    if (props.action === 'edit') {
      const res: any = await pjForeignupdate(value);
      if (res.code == 200) {
        page.closeModal();
        setTimeout(() => {
          history.go(0);
    ...
    if (props.action === 'add') {
      const res: any = await pjForeignadd(value);
      if (res.code == 200) {
        page.closeModal();
        setTimeout(() => {
          history.go(0);
  ...
  return (
    <BaseEdit
      ...
      submitter={check('/ticket/foreign/commit') === false ? false : true}
