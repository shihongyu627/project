...
const Page: React.FC<any> = (props) => {
  ...
  const getData = async () => {
    const result: any = await selectDetails();
  ...
  const onSubmit = async (values: any) => {
    ...
    if (value.id) {
      ...
      // console.log('checkIdName1', rescheckIdName);
  ...
  return (
    <BaseEdit
      pageProps={{
        title: (props.action || props?.location?.query?.action) == 'add' ? '新增客户' : '企业管理',
      }}
      {...props}
      mode={'page'}
      rowCol={2}
      ...
      submitter={
        check('/ticket/enterprise/commit') === false
          ? {
              render() {
                return [];
              },
