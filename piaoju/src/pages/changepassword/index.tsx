...
import { changePassword } from '@/services/ant-design-pro/account';

const Page: React.FC<any> = (props) => {
  const [formLayoutType, setFormLayoutType] = React.useState<'horizontal' | 'vertical' | 'inline'>(
    'vertical',
  );
  const [rowCol, setRowCol] = React.useState<number>(2);
  ...
  const onSubmits = async (value: any) => {
    if (value.newPassword === value.confirmPassword) {
      // console.log('onSubmits:', value);
      const a: any = JSON.parse(`${localStorage.getItem('userInfo')}`);
      // console.log('onSubmits:', a);
      value.id = a.id;
      value.username = a.username;
      const res: any = await changePassword(value);
      // console.log('onSubmits:', res);
      if (res.code == 200) {
        history.back();
      }
    } else {
